...
stopwords_path = os.getcwd() + '/pheme_files_vol3/stopwords_eng1.txt'
stopwords_eng1 = []
with open(stopwords_path, 'r') as f:
    for line in f.readlines():
        stopwords_eng1.append(line.strip())

stopwords_path = os.getcwd() + '/pheme_files_vol3/stopwords_eng2.txt'
stopwords_eng2 = []
with open(stopwords_path, 'r') as f:
    for line in f.readlines():
...
def get_vectors(text):
    inputs = tokenizer.encode(text, add_special_tokens=True, padding=True, truncation=True, return_tensors="pt")

    with torch.no_grad():
    # 使用BERT模型编码输入句子
        outputs = model(inputs)
        # 获取句子的向量表示，通常是[CLS]标记对应的向量
...
def get_graphs():
    links = []
    ts = []
    with open(root_path + '/pheme_files_vol3/edge.json','r',encoding='utf-8') as input:
    ...
    #     if i == 0:
    #         plt.savefig( './pheme_files_vol3/early.png')
    #     elif i == 1:
    #         plt.savefig( './pheme_files_vol3/mid.png')
    #     elif i == 2:
    #         plt.savefig( './pheme_files_vol3/last.png')
        #plt.show()
...
def read_corpus(root_path, file_name):
    ...
    with open(root_path + '/pheme_files_vol3/node_id_new.json','w',encoding='utf-8') as output:
    ...
    with open(root_path + '/pheme_files_vol3/comment_content.json','r',encoding='utf-8') as input:
        comment_map = {}
    ...
    with open(root_path + '/pheme_files_vol3/user_tweet.json','r',encoding='utf-8') as input:
        old_user_post_map = {}
    ...
    node_embedding_matrix = np.zeros((len(index_dict), 768))
    ...
    pickle.dump([node_embedding_matrix],
                open(root_path + "/pheme_files_vol3/node_embedding.pkl", 'wb'))
    ...
    # save
    with open(root_path + '/pheme_files_vol3/id_post_new.json','w',encoding='utf-8') as output:
        json.dump(old_id_post_map,output,ensure_ascii=False,indent=4)
    with open(root_path + '/pheme_files_vol3/id_comment_new.json','w',encoding='utf-8') as output:
        json.dump(comment_map,output,ensure_ascii=False,indent=4)
        
    save_path = "./pheme_files_vol3/graphs_new.pkl" #是否可以理解为替代上面的node_embedding_matrix？好像是
...
def feature_extract(root_path,filename,w2v_path):
    ...
    pickle.dump([X_train_tid, X_train, y_train], open(root_path + "/pheme_files_vol3/train.pkl", 'wb'))
    pickle.dump([X_dev_tid, X_dev, y_dev], open(root_path + "/pheme_files_vol3/dev.pkl", 'wb'))
    pickle.dump([X_test_tid, X_test, y_test], open(root_path + "/pheme_files_vol3/test.pkl", 'wb'))
...
if __name__ == "__main__":
    ...
    #     exec(f.read())
    root_path = os.getcwd()
    filename = '/pheme_files_vol3/'
    feature_extract(root_path=root_path, filename=filename,w2v_path=os.getcwd() + "/pheme_files_vol3/twitter_w2v.bin")