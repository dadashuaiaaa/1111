...
def get_vectors(text):
    ...
    with torch.no_grad():
        ...
        sentence_embedding = outputs.last_hidden_state.mean(dim=1)  # 使用均值池化获取句子向量
...
def get_graphs():
    links = []
    ts = []
    with open(root_path + '/weibo_files_vol2/edge.json','r',encoding='utf-8') as input:
...
def read_corpus(root_path, file_name):
    ...
    with open(root_path + '/weibo_files_vol2/node_id_new.json','w',encoding='utf-8') as output:
    ...
    with open(root_path + '/weibo_files_vol2/comment_content.json','r',encoding='utf-8') as input:
        comment_map = {}
    ...
    with open(root_path + '/weibo_files_vol2/user_tweet.json','r',encoding='utf-8') as input:
        old_user_post_map = {}
    ...
    pickle.dump([node_embedding_matrix],
                open(root_path + "/weibo_files_vol2/node_embedding.pkl", 'wb'))
    ...
    # save
    with open(root_path + '/weibo_files_vol2/id_post_new.json','w',encoding='utf-8') as output:
        json.dump(old_id_post_map,output,ensure_ascii=False,indent=4)
    with open(root_path + '/weibo_files_vol2/id_comment_new.json','w',encoding='utf-8') as output:
        json.dump(comment_map,output,ensure_ascii=False,indent=4)
        
    save_path = "./weibo_files_vol2/graphs_new.pkl" #是否可以理解为替代上面的node_embedding_matrix？好像是
...
def w2v_feature_extract(root_path, filename):
    ...
    pickle.dump([X_train_tid, X_train, y_train], open(root_path + "/weibo_files_vol2/train.pkl", 'wb'))
    pickle.dump([X_dev_tid, X_dev, y_dev], open(root_path + "/weibo_files_vol2/dev.pkl", 'wb'))
    pickle.dump([X_test_tid, X_test, y_test], open(root_path + "/weibo_files_vol2/test.pkl", 'wb'))
...
if __name__ == "__main__":
    ...
    #     exec(f.read())
    root_path = os.getcwd()
    filename = '/weibo_files_vol2/weibo'
