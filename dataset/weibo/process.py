...
maxlen = 35
use_stopwords = True
stopwords_path = os.getcwd() + '/weibo_files_vol1/stopwords.txt'
stopwords = []
...
#     text = re.sub(r'来自话题', '', text)
#     text = re.sub(r'转发微博', '', text)
#     text = re.sub(r'话题详情', '', text)
...
def get_origin7rumor7non_rumor(data_root_dir='download/weibo2', min_review_length=150):
    ...
    for file in origin_file_names:
        ...
        nodetonode = str(user) + '_' + str(weibo_name)
        create_time = "2010-12-23 10:00:00" #最早时间
        parsed_time = datetime.strptime(create_time, "%Y-%m-%d %H:%M:%S")
        create_time = parsed_time.strftime("%Y-%m-%dT%H:%M:%S.%f+0000")
    ...
    for file in rumor_file_names:
        ...
        with open(path, 'r', encoding='utf-8') as f:
            reviews = json.load(f)
            rumor_mid_reviews_map[rumor_name] = select_reviews(rumor_name, reviews, min_review_length,review_content_map,edge)
    ...
    for file in non_rumor_file_names:
        ...
        with open(path, 'r', encoding='utf-8') as f:
            reviews = json.load(f)
            non_rumor_mid_reviews_map[non_rumor_name] = select_reviews(non_rumor_name,reviews,min_review_length,review_content_map,edge)

    # new_id_mapping = {}
    # for i in range(0,len(nodes)):
    #     new_id_mapping[nodes[i]] = i

    with open(os.getcwd() + '/weibo_files_vol1/edge.json', 'w') as f:
        f.write(json.dumps(edge, indent=4))
    # with open(os.getcwd() + '/weibo_files_vol1/node_id.json', "w", encoding="utf-8") as output_file:
    ...
    return origin_mid_json_map, rumor_mid_reviews_map, non_rumor_mid_reviews_map, user_weibo_map,\
            weibo_content_map,review_content_map,edge

def select_reviews(rumor_name, reviews, min_review_length,review_content_map,edge):
    review_list = []
    # print(rumor_name)
    for review in reviews:
        if review['parent'] == '' and len(review['text']) > min_review_length:
            nodetonode = str(rumor_name) + '_' + str(review['mid'])
...
def create_input_files(data_root_dir='download/weibo2', ratio=[70,10,20], data_set_name='weibo', min_review_length=150):
    origin, rumor, non_rumor, user_weibo_map_old, weibo_content_map,review_content_map,edge= \
    ...
    with open(os.getcwd() + '/weibo_files_vol1/comment_content.json', 'w') as f:
        f.write(json.dumps(review_content_map, indent=4))
    with open(os.getcwd() + '/weibo_files_vol1/user_tweet.json', 'w') as f:
    ...
    write2file(data_set_name, weibo_content_map, weibo_label_map, adj_node_map,
               ...
               suffix='_graph.txt')
    path = os.getcwd() + '/weibo_files_vol1/original_adj'
    adj_node_map_tmp = {}
...
def write2file(data_set_name, weibo_content_map, weibo_label_map, adj_node_map, name_list,
               suffix='.train'):
    path = os.getcwd() + '/weibo_files_vol1/' + data_set_name + suffix
...
def main():
    ...
    ratio = [70,10,20]
    create_input_files(data_root_dir=data_dir, ratio=ratio, data_set_name='weibo', min_review_length=70)
