...
class NeuralNetwork(nn.Module):
    ...
    def fit(self, X_train_tid, X_train, y_train,
            X_dev_tid, X_dev, y_dev,train_content,dev_content):
        ...
        for epoch in range(self.config['epochs']):
            ...
            self.evaluate(X_dev_tid, X_dev, y_dev,dev_content)
...
tokenizer = AutoTokenizer.from_pretrained(r'E:\Mango\代码\Bert2\bert-base-chinese')
...
def load_dataset():
    ...
    dev_content=[]
    test_content=[]
    with open( "E:/NLP/MFAN-main/dataset/pheme/pheme_files/pheme.train", 'r', encoding='utf-8') as input:
        for line in input.readlines():
            tid, content, label = line.strip().split("\t")
            train_content.append(content)
    with open("E:/NLP/MFAN-main/dataset/pheme/pheme_files/pheme.dev", 'r', encoding='utf-8') as input:
        for line in input.readlines():
            tid, content, label = line.strip().split("\t")
            dev_content.append(content)
    with open("E:/NLP/MFAN-main/dataset/pheme/pheme_files/pheme.test", 'r', encoding='utf-8') as input:
        for line in input.readlines():
            tid, content, label = line.strip().split("\t")
    ...
    return X_train_tid, X_train, y_train, \
           ...
           X_test_tid, X_test, y_test, adj,train_content,dev_content,test_content
...
def train_and_test(model):
    ...
    X_dev_tid, X_dev, y_dev, \
    X_test_tid, X_test, y_test, adj,train_content,dev_content,test_content= load_dataset()
    ...
    y_pred = nn.predict(X_test_tid, X_test,y_test,test_content)
