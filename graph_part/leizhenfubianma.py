...
def quantum_graph_layer_QGRNN(node_features, weights, coupling):
    ...
    qml.templates.AmplitudeEmbedding(node_features[ :8], wires=[0, 1, 2], normalize=True)
    qml.templates.AmplitudeEmbedding(node_features[ 8:16], wires=[3, 4, 5], normalize=True)
    qml.templates.AmplitudeEmbedding(node_features[ 16:24], wires=[6, 7, 8], normalize=True)

    #qml.MottonenStatePreparation(node_features[:, :8], wires=[0, 1, 2])
    #qml.MottonenStatePreparation(node_features[:, 8:16], wires=[3, 4, 5])
    #qml.MottonenStatePreparation(node_features[:, 8:16], wires=[6, 7, 8])
...
class QGRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        results = [quantum_graph_layer_QGRNN(features, self.weights,self.coupling) for features in x]
