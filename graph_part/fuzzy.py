...
# 定义时间演化算符
def time_evolution(weights, delta):
    # 将信息从前四个量子比特传递到中间四个量子比特

    for i in range(4):
        for j in range(4, 8):
            qml.CNOT(wires=[i, j])
            qml.RZ(-2 * delta * weights[i], wires=j)

    # 将信息从中间四个量子比特传递到后四个量子比特
    for i in range(4, 8):
        for j in range(8, 12):
            qml.CNOT(wires=[i, j])
            qml.RZ(-2 * delta * weights[i], wires=j)
...
def quantum_graph_layer_QGRNN(node_features, weights, coupling):

    qml.RY(node_features[:, :1].squeeze(), wires=0)
    qml.RZ(node_features[:, 1:2].squeeze(), wires=0)
    qml.RY(node_features[:, 2:3].squeeze(), wires=1)
    qml.RZ(node_features[:, 3:4].squeeze(), wires=1)
    qml.RY(node_features[:, 4:5].squeeze(), wires=2)
    qml.RZ(node_features[:, 5:6].squeeze(), wires=2)
    qml.RY(node_features[:, 6:7].squeeze(), wires=3)
    qml.RZ(node_features[:, 7:8].squeeze(), wires=3)
    qml.RY(node_features[:, 8:9].squeeze(), wires=4)
    qml.RZ(node_features[:, 9:10].squeeze(), wires=4)
    qml.RY(node_features[:, 10:11].squeeze(), wires=5)
    qml.RZ(node_features[:, 11:12].squeeze(), wires=5)
    qml.RY(node_features[:, 12:13].squeeze(), wires=6)
    qml.RZ(node_features[:, 13:14].squeeze(), wires=6)
    qml.RY(node_features[:, 14:15].squeeze(), wires=7)
    qml.RZ(node_features[:, 15:16].squeeze(), wires=7)
    qml.RY(node_features[:, 16:17].squeeze(), wires=8)
    qml.RZ(node_features[:, 17:18].squeeze(), wires=8)
    qml.RY(node_features[:, 18:19].squeeze(), wires=9)
    qml.RZ(node_features[:, 19:20].squeeze(), wires=9)
    qml.RY(node_features[:, 20:21].squeeze(), wires=10)
    qml.RZ(node_features[:, 21:22].squeeze(), wires=10)
    qml.RY(node_features[:, 22:23].squeeze(), wires=11)
    print(node_features[:, 22:23].shape)

    qml.RZ(node_features[:, 23:24].squeeze(), wires=11)
    #for j in range(n_qubits):
        #qml.RY(node_features[:, 2 * j], wires=j)
        #print(node_features[:, 2 * j].shape)
        #qml.RZ(node_features[:, 2 * j + 1], wires=j)

        # qml.AngleEmbedding(node_features[:, :4], wires=[0, 1, 2, 3])
        # qml.AngleEmbedding(node_features[:, 4:8], wires=[4, 5, 6, 7])
        # qml.AngleEmbedding(node_features[:, :-8], wires=[8, 9, 10, 11])
        # 单量子比特旋转
    ...
    for _ in range(n_steps):
        time_evolution(weights, time_step)
...
# 定义QCNN为PyTorch模块
class QGRNN(nn.Module):
    def __init__(self, n_qubits, weights, coupling):
        super(QGRNN, self).__init__()
    ...
    def forward(self, x):
        ...
        q_out = quantum_graph_layer_QGRNN(x, self.weights, self.coupling)
...
class NerualNetwork(nn.Module):
    ...
    def mdegcn(self, x_tid, y, loss, i, total):
        ...
        #print(torch.max(logit_origin, 1)[1].view(y.size()).data)
    ...
    def fit(self, X_train_tid, y_train, X_dev_tid, y_dev,X_test_tid, y_test):
        ...
        self.optimizer = torch.optim.Adam(self.parameters(), lr=1e-5, weight_decay=config['weight_decay'])
        ...
        #epoch_iterator = tqdm(range(self.config['epochs']), desc="Epochs", unit="epoch")
        ...
        for epoch in range(self.config['epochs']):
            ...
            with tqdm(total=len(dataloader), leave=True) as pbar:
                for i, data in enumerate(dataloader):
                    ...
                    self.mdegcn(batch_x_tid, batch_y, loss, i, total=len(dataloader))
...
class MDEGCN(NerualNetwork):
    def __init__(self, config):
        ...
        qbits = 8  # 假设需要传递的qbits参数为8，你需要根据实际情况进行调整
        weights = torch.tensor(np.random.random(3 * n_qubits), requires_grad=True)
        coupling = torch.tensor(np.random.random((n_qubits, n_qubits)), requires_grad=True)
        self.qgrnn = QGRNN(qbits, weights, coupling)
        #self.qnn = QCNN()
        self.Net = Net
    ...
    def forward(self, X_train_tid):
        ...
        graph_feature = all_graph_feature[X_train_tid]
        graph_feature= self.fc8(graph_feature)

        graph_feature = graph_feature.view(graph_feature.shape[0], -1)
