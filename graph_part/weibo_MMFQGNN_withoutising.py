...
def quantum_graph_layer_QGRNN(node_features, weights, coupling):
    ...
    return [qml.expval(qml.PauliZ(j)) for j in range(qbits2)]
...
class NerualNetwork(nn.Module):
    ...
    def fit(self, X_train_tid, y_train, X_dev_tid, y_dev,X_test_tid, y_test):
        ...
        self.optimizer = torch.optim.Adam(self.parameters(), lr=5e-3, weight_decay=config['weight_decay'])
...
class MDEGCN(NerualNetwork):
    ...
    def forward(self, X_train_tid):
        ...
        # self.qcnn.to(device)
        q_input = torch.cat((textual_feature, visual_feature, Qgraph_feature), dim=1)
