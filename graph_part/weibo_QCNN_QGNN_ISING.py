...
# Pennylane设备
dev = qml.device("default.qubit", wires=qbits2)
...
def quantum_graph_layer_QGRNN(node_features, weights, coupling):
    ...
    # 单量子比特旋转
    for i in range(qbits2):
        qml.RX(weights[i], wires=i)
        qml.RY(weights[i + qbits2], wires=i)
        qml.RZ(weights[i + 2 * qbits2], wires=i)
    ...
    #for i in range(n_qubits):
        #for j in range(i + 1, n_qubits):
           # qml.CNOT(wires=[i, j])
            ...
            #qml.CNOT(wires=[i, j])
    for i in range(qbits2):
        for j in range(i + 1, qbits2):
            qml.IsingXX(coupling[i, j], wires=[i, j])
    # 返回每个量子比特的测量值
    return [qml.expval(qml.PauliZ(i)) for i in range(qbits2)]
...
class MDEGCN(NerualNetwork):
    def __init__(self, config):
        ...
        qbits1 = 8  # 假设需要传递的qbits参数为8，你需要根据实际情况进行调整
        self.qcnn = QCNN(qbits1)
        qbits2 = 12
        weights2 = torch.tensor(np.random.random(3 * qbits2), requires_grad=True)
        coupling2 = torch.tensor(np.random.random((qbits2, qbits2)), requires_grad=True)
        self.qgrnn = QGRNN(qbits2, weights2, coupling2)


        qbits3 = 4
        weights3 = torch.tensor(np.random.random(3 * qbits3), requires_grad=True)
        ...
        self.fc5 = nn.Linear(1048, 512)
        self.fc6 = nn.Linear(512, 256)
        self.fc7 = nn.Linear(12, 24)
    ...
    def forward(self, X_train_tid):
        ...
        qgrnn=self.qgrnn
        qgrnn.to(device)
        Qgraph_feature = qgrnn(graph_feature)
        #print(outputs.shape)
        Qgraph_feature = Qgraph_feature.to(torch.float32)
        #Qgraph_feature = self.fc7(Qgraph_feature)
        ...
        # self.qcnn.to(device)
        textual_feature = textual_feature.float()
        visual_feature = visual_feature.float()

        textual_feature = self.fc9(textual_feature)
        visual_feature = self.fc9(visual_feature)


        qcnn = self.qcnn
        qcnn.to(device)
        Qtextual_feature = qcnn(textual_feature)
        Qvisual_feature = qcnn(visual_feature)


        q_input = torch.cat((Qtextual_feature, Qvisual_feature, Qgraph_feature), dim=1)
